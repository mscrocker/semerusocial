...
public class FriendServiceImpl implements FriendService {
  ...
  public Block<FriendListOut> getFriendList(Long userId, int page, int size)
    ...
    final Slice<MatchImpl> friendsResult = matchRepository.findFriends(userId, PageRequest.of(page,
        size));
  ...
  private void validateRecommendation(Long subject, Long object) throws InstanceNotFoundException,
    ...
    final Optional<RejectedImpl> optRejected = rejectedRepository.findById(new RejectedId(subject,
        object));
    ...
    final Optional<RequestImpl> optAcepted = requestRepository.findById(new RequestId(subject,
        object));
  ...
  private boolean matchesCriteria(UserImpl criteria, UserImpl user)
  ...
  public void blockUser(Long userId, Long friendId)
    ...
    final BlockedImpl block2 = new BlockedImpl(new BlockedId(userId, friendId),
        LocalDateTime.now());
  ...
  public SuggestedSearchCriteria suggestNewCriteria(Long userId)
    ...
    for (int minRate = searchCriteria.getMinRate(); minRate >= minPosibleRate; minRate -= 1) {
      ...
      SearchCriteria newCriteria = new SearchCriteria(searchCriteria.getSex(),
          searchCriteria.getMinAge(),
      ...
      if (newUsersSuggestedLimit > 0) { // Si maxAge=200 encuentra a alguien
        for (int maxAge = searchCriteria.getMaxAge(); maxAge <= maxPosibleAge; maxAge += 5) {
          newCriteria = new SearchCriteria(searchCriteria.getSex(), searchCriteria.getMinAge(),
              maxAge, searchCriteria.getCity(), minRate);
        ...
        return new SuggestedSearchCriteria(0,
            maxPosibleAge - searchCriteria.getMaxAge(), 0,
            newUsersSuggestedLimit);
      }
      // minAge
      newCriteria = new SearchCriteria(searchCriteria.getSex(), minPosibleAge,
      ...
      if (newUsersSuggestedLimit > 0) { // Si minAge = 18 encuentra a alguien
        for (int minAge = searchCriteria.getMinAge(); minAge >= minPosibleAge; minAge -= 5) {
          ...
          newUsersSuggested = userRepository.findByCriteriaMaxResults(newCriteria, userId);
          if (newUsersSuggested > 0) {
            return new SuggestedSearchCriteria(
                newCriteria.getMinAge() - searchCriteria.getMinAge(), 0,
                minRate - searchCriteria.getMinRate(), newUsersSuggested);
          }
        } // Para el caso del minimo
        return new SuggestedSearchCriteria(minPosibleAge - searchCriteria.getMinAge(),
            0, 0, newUsersSuggestedLimit);
      }
    }
    throw new CantFindMoreFriendsException("Even if you change your criteria you cant find more "
        + "friends");
  ...
  public Block<UserImpl> searchUsersByMetadataAndKeywords(SearchUsersDto params, int page,
                                                          int size) {
    final Slice<UserImpl> users = userRepository.searchUsersByMetadataAndKeywords(params, page,
        size);
