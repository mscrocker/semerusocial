...
public class ChatServiceImpl implements ChatService {
  ...
  public Block<MessageDetailsDto> getConversation(Long userId, Long friendId, int page, int size)
    ...
    if (userId < friendId) {
      conversation = messageRepository.findMessagesByUsersId(userId, friendId, PageRequest.of(page,
          size));
    } else {
      conversation = messageRepository.findMessagesByUsersId(friendId, userId, PageRequest.of(page,
          size));
    }
  ...
  public Block<FriendChatTitle> getUserConversations(Long userId, int page)
    ...
    final Slice<FriendChatTitle> result = messageRepository.getLatestConversations(userId, page,
        10);
