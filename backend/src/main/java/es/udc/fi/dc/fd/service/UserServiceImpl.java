...
public class UserServiceImpl implements UserService {
  ...
  public void setSearchCriteria(Long userId, SearchCriteria criteria)
    ...
    if (criteria.getMinAge() < 18) {
      throw new InvalidAgeException("Age must be higher than 18 years, not "
          + criteria.getMinAge());
    }
    if (criteria.getMinAge() > criteria.getMaxAge()) {
      throw new InvalidAgeException(
          "MinAge must be lower than MaxAge : " + criteria.getMinAge() + " > "
              + criteria.getMaxAge());
    }
    ...
    for (final String city : toDeleteList) {
      final CityCriteriaImpl cityCriteriaId = new CityCriteriaImpl(new CityCriteriaId(userId,
          city));
  ...
  public void updateProfile(Long userId, UserImpl user) throws InstanceNotFoundException,
    ...
    if (!userFound.isPresent()) {
      throw new InstanceNotFoundException("User with userId=" + userId + " doesn't exist",
          userFound);
    }
  ...
  public double rateUser(int rate, Long userSubject, Long userObject)
    ...
    if (userOptional.isPresent()) { // Si el usuario al que se vota existe
      ...
      final Optional<RateImpl> rateOptional = rateRepository.findById(new RateId(userSubject,
          userObject));
      ...
      if (!rateOptional.isPresent()) { // Si el usuario no habia votado añadimos una nueva fila
        ...
        if (newRateImpl.getPoints() == rate) { // Si hace la misma votacion la media se queda igual
          ...
          newRate = (totalVotes - newRateImpl.getPoints() + rate) / userVotes; // Calculamos la
          // nueva media
  ...
  public Block<UserImpl> getTopUsers(String city, int page, int size) {
    ...
    final Slice<UserImpl> users = userRepository.findByCityOrderByRatingDesc(city,
